    ...
    <div class="flex items-center justify-between">
      ...
      <div v-if="invoice">
        <button @click="downloadPDF"
          class="flex items-center px-4 py-2 bg-[#CF0D0F] text-white rounded-lg hover:bg-[#F6211F] transition">
...
import { ArrowLeft, AlertCircle, Download } from 'lucide-vue-next';
...
const loadInvoice = async () => {
  ...
  try {
    const response = await staffDashboard.getInvoice(route.params.id);
    if (response.success) {
...
const downloadPDF = () => {
  const url = `${import.meta.env.VITE_API_BASE_URL}/api/v1/staff/invoices/${invoice.value.id}/pdf`;
