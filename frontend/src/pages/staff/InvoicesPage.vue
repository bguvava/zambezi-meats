          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="invoice in invoices" :key="invoice.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button @click="viewInvoice(invoice.id)" class="text-blue-600 hover:text-blue-900" title="View details">
                  <Eye class="h-5 w-5" />
                </button>
              </td>
            </tr>
...
const loadInvoices = async () => {
  ...
  try {
    ...
    const response = await staffDashboard.getInvoices(params);
...
const loadStats = async () => {
  try {
    const response = await staffDashboard.getInvoiceStats();
    if (response.success) {
...
const viewInvoice = (id) => {
  router.push({ name: 'staff-invoice-detail', params: { id } });
};
